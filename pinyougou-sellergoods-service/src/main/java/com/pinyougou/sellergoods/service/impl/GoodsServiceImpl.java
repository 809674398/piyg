...
public class GoodsServiceImpl implements GoodsService {
	...
	public void add(Goods goods) {
		goods.getGoods().setAuditStatus("0");//设置商品未审核
		goodsMapper.insert(goods.getGoods());//插入商品
		goods.getGoodsDesc().setGoodsId(goods.getGoods().getId());//获取插入商品的Id
		goodsDescMapper.insert(goods.getGoodsDesc());//插入商品详情
		saveItemList(goods);
	...
	private void setItemValues(TbItem item,Goods goods){
        ...
        //构建标题
        String title = goods.getGoods().getGoodsName();//获取SPU名称
        ...
        item.setSellerId(goods.getGoods().getSellerId());//设置商家Id
        item.setGoodsId(goods.getGoods().getId());//设置商品Id
	...
	public void update(Goods goods){
		goods.getGoods().setAuditStatus("0");//设置修改的商品为未审核状态
		goodsMapper.updateByPrimaryKey(goods.getGoods());//修改商品表
		goodsDescMapper.updateByPrimaryKey(goods.getGoodsDesc());//修改商品扩展表
		//删除原有的sku表
	...
	public PageResult findPage(TbGoods goods, int pageNum, int pageSize) {
		...
		criteria.andIsDeleteIsNull();//加上条件判断,isDelete属性为NULL才进行查询
		if(goods!=null){			
